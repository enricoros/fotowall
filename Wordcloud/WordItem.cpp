...
WordItem::WordItem(const Word & word,
                   const QFont & font,
                   double rotation,
                   int minCount,
                   int maxCount,
                   QGraphicsItem * parent)
: QAbstractGraphicsShapeItem(parent), m_font(font), m_rotation(rotation), m_string(word.commonString)
{
  // customize item
...
: QAbstractGraphicsShapeItem(parent), m_rotation((qreal)0.0)
{
  // customize item
  setFlags(QGraphicsItem::ItemIsMovable | QGraphicsItem::ItemIsFocusable | QGraphicsItem::ItemIsSelectable);
#if QT_VERSION >= 0x040600
  ...
  if(!family.isEmpty()) m_font.setFamily(family);
...
void WordItem::saveToXml(QDomElement & wordElement) const
{
  ...
  // SIMPLE save brush
  if(!brush().color().isValid()) qWarning("WordItem::saveToXml: implement complex brush xml dump");
...
QPainterPath WordItem::strokedPath() const
{
  ...
  if(m_path == QPainterPath() || pen.widthF() <= 0.0) return m_path;
