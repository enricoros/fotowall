...
PE_AbstractSlider::PE_AbstractSlider(QAbstractSlider * _slider,
                                     QObject * _target,
...
: PE_TypeControl<QAbstractSlider>(_slider, _target, propertyName, parent)
{
  ...
  if(m_property.type() != QVariant::Int && m_property.type() != QVariant::Double
     && m_property.type() != SYMBIAN_STRANGE_VARIANT)
...
void PE_AbstractSlider::slotSliderValueChanged(int intValue)
{
  ...
  if(!m_control || !m_target) return;

  // QVariant::Int: properties are directly linked to the slider
  if(m_property.type() == QVariant::Int) { m_property.write(m_target.data(), intValue); }
  ...
  else if(m_property.type() == QVariant::Double || m_property.type() == SYMBIAN_STRANGE_VARIANT)
  {
    ...
    else if(m_control->maximum() > m_control->minimum())
    {
      ...
      qreal realVal =
          (qreal)(intValue - m_control->minimum() + 1) / (qreal)(m_control->maximum() - m_control->minimum() + 1);
#else
...
PE_AbstractButton::PE_AbstractButton(QAbstractButton * _button,
                                     QObject * _target,
                                     const char * propertyName,
                                     QObject * parent)
...
{
  // set the property to the current state of the button
  if(m_control && m_target && (m_property.type() == QVariant::Int)) m_property.write(m_target.data(), intValue);
}
